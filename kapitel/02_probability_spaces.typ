...
#remark[
Suppose $Omega = [0, 1]$ and we want a "uniform" $PP$ with $PP([a, b]) = b - a$. It is a deep theorem (related to the work of Vitali, Banach and Tarski) that #emph[no] such $PP$ can be defined on #emph[all] of $cal(P)([0, 1])$ while also being non-negative, normalized, countably additive and translation invariant: there exist pathological *non-measurable sets* to which no consistent "length" can be assigned. The fix is pragmatic: restrict attention to a smaller — but still very rich — collection of "well-behaved" subsets. That collection is a $sigma$-algebra.
]
...
$
emptyset in cal(A), quad A union B in cal(A), quad A inter B in cal(A), quad A backslash B in cal(A),
$
...
#remark[
Equivalently $cal(B)^n = sigma(cal(O)^n)$, the $sigma$-algebra generated by the open sets (the standard topology). The same $cal(B)^n$ is obtained from many other generators — half-open intervals with rational endpoints, closed sets, compact sets, rays $(-oo, b]$, and so on. Crucially $cal(B)^n$ contains #emph[every] set one ever meets in practice (all open, closed, countable sets, all intervals) yet is strictly smaller than $cal(P)(RR^n)$: the pathological non-measurable sets are exactly what has been left out.
]
...
Inclusion--exclusion generalizes to any finite number of events, the sign alternating with the size of the intersection.
...
#remark[
On a finite space with the *uniform* (Laplace) measure, every computation collapses to counting: $PP(A) = |A| \/ |Omega|$, so probability questions become combinatorics. The axiomatic machinery only truly earns its keep once $Omega$ is uncountable, where singletons have probability $0$ yet events still carry positive probability — the theme of Chapters 3 and 4.
]
