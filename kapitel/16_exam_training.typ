...
(iv) Let $X_1, X_2, dots$ be iid with $EE[X_i] = 0$ and $"Var"(X_i) = 1$, and let $overline(X)_n = 1/n sum_(i=1)^n X_i$. Then $F_(overline(X)_n)(x) = Phi(x)$ for all $n$, where $Phi$ is the standard normal cdf.
]
#answer[
...
*(iv) False.* The equality $F_(overline(X)_n) = Phi$ is what the central limit theorem gives only in the limit $n -> oo$, and even then not for $overline(X)_n$ itself: the correctly scaled quantity is $sqrt(n) overline(X)_n$ (here $Z_n = (sum_i X_i)/sqrt(n)$). For finite $n$ the cdf of $overline(X)_n$ is generally not standard normal.
]
...
#answer[
*(i) Strong law of large numbers.* Let $X_1, X_2, dots$ be a sequence of iid random variables with finite mean $EE[X_i] = mu < oo$, and let $overline(X)_n = 1/n sum_(i=1)^n X_i$ be the sample mean. Then
$
...
$
Z_n = (S_n - EE[S_n])/sqrt("Var"(S_n)) = (S_n - n mu)/(sigma sqrt(n)) = (overline(X)_n - mu)/(sigma \/ sqrt(n)) ->^("d") cal(N)(0, 1) .
$
Equivalent standard forms are $S_n approx cal(N)(n mu, n sigma^2)$ and $overline(X)_n approx cal(N)(mu, sigma^2 \/ n)$ (informal large-$n$ approximations only, since the right-hand side depends on $n$ and is therefore not a fixed limit law); the precise convergence-in-distribution statement is the standardized one, $sqrt(n)(overline(X)_n - mu) ->^("d") cal(N)(0, sigma^2)$.
...
(d) Historically a light-bulb line averages $lambda_0 = 4$ defectives per batch. Testing $n = 100$ new batches gives $overline(x)_100 = 5$. Construct an approximate level-$alpha = 0.05$ test of whether the mean #emph[increased], and decide. (Use $z_(0.05) approx 1.64$.)
]
#answer[
...
$
(d ell)/(d lambda) = -n + 1/lambda sum_i x_i = 0 quad ==> quad lambda = 1/n sum_(i=1)^n x_i = overline(x)_n .
$
The second derivative $-(sum_i x_i)\/lambda^2 < 0$ confirms a maximum, so $hat(lambda)_"mle" = overline(X)_n$.

*(b) Yes.* MLEs are not unbiased in general, but here the estimator is the sample mean, which is unbiased for $EE[X_i] = lambda$: $EE[overline(X)_n] = lambda$.

*(c)* Since $EE[X_i] = "Var"(X_i) = lambda$, the CLT gives, for large $n$,
$
overline(X)_n approx cal(N)(EE[X_i], ("Var"(X_i))/n) = cal(N)(lambda, lambda\/n) .
$

*(d)* Test $H_0 : lambda = 4$ against the one-sided $H_1 : lambda > 4$. Under $H_0$, $hat(lambda)_"mle" = overline(X)_n approx cal(N)(4, 4\/n)$, so the standardized statistic is
$
Z = (overline(X)_n - 4)/sqrt(4\/100) approx cal(N)(0, 1) quad "under" H_0 .
$
...
(d) For $X_1, X_2, dots ~ "Poi"(2)$ the sample mean satisfies $overline(X)_n ->^("P") 2$.
]
#answer[
...
*(d) False.* The statement is missing #emph[independence]. Without it the weak law can fail: if $X_i = X_1$ for all $i$, then $overline(X)_n = X_1$ for every $n$, which does not converge to the constant $2$ (indeed $PP(|X_1 - 2| >= epsilon) > 0$ for suitable $epsilon$, since $PP(X_1 = k) > 0$ for all $k in NN$).
]
