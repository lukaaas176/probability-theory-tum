...
- *Estimator questions.* MLE: write the likelihood, take $log$, differentiate, check the second derivative. Then check unbiasedness by taking an expectation, and use the CLT for the approximate sampling distribution.
]
...
(iv) Let $X_1, X_2, dots$ be iid with $EE[X_i] = 0$ and $"Var"(X_i) = 1$, and let $bar(X)_n = 1/n sum_(i=1)^n X_i$. Then $F_(bar(X)_n)(x) = Phi(x)$ for all $n$, where $Phi$ is the standard normal cdf.
]
#answer[
...
*(iv) False.* The equality $F_(bar(X)_n) = Phi$ is what the central limit theorem gives only in the limit $n -> oo$, and even then not for $bar(X)_n$ itself: the correctly scaled quantity is $sqrt(n) bar(X)_n$ (here $Z_n = (sum_i X_i)/sqrt(n)$). For finite $n$ the cdf of $bar(X)_n$ is generally not standard normal.
]
...
#answer[
*(i) Strong law of large numbers.* Let $X_1, X_2, dots$ be a sequence of iid random variables with finite mean $EE[X_i] = mu < oo$, and let $bar(X)_n = 1/n sum_(i=1)^n X_i$ be the sample mean. Then
$
bar(X)_n ->^("a.s.") mu .
$
...
$
Z_n = (S_n - EE[S_n])/sqrt("Var"(S_n)) = (S_n - n mu)/(sigma sqrt(n)) = (bar(X)_n - mu)/(sigma \/ sqrt(n)) ->^("d") cal(N)(0, 1) .
$
Equivalent standard forms are $S_n ->^("d") cal(N)(n mu, n sigma^2)$, $bar(X)_n ->^("d") cal(N)(mu, sigma^2 \/ n)$, and $sqrt(n)(bar(X)_n - mu) ->^("d") cal(N)(0, sigma^2)$.
...
(d) Historically a light-bulb line averages $lambda_0 = 4$ defectives per batch. Testing $n = 100$ new batches gives $bar(x)_100 = 5$. Construct an approximate level-$alpha = 0.05$ test of whether the mean #emph[increased], and decide. (Use $z_(0.05) approx 1.64$.)
]
#answer[
...
$
(d ell)/(d lambda) = -n + 1/lambda sum_i x_i = 0 quad ==> quad lambda = 1/n sum_(i=1)^n x_i = bar(x)_n .
$
The second derivative $-(sum_i x_i)\/lambda^2 < 0$ confirms a maximum, so $hat(lambda)_"mle" = bar(X)_n$.

*(b) Yes.* MLEs are not unbiased in general, but here the estimator is the sample mean, which is unbiased for $EE[X_i] = lambda$: $EE[bar(X)_n] = lambda$.

*(c)* Since $EE[X_i] = "Var"(X_i) = lambda$, the CLT gives, for large $n$,
$
bar(X)_n approx cal(N)(EE[X_i], ("Var"(X_i))/n) = cal(N)(lambda, lambda\/n) .
$

*(d)* Test $H_0 : lambda = 4$ against the one-sided $H_1 : lambda > 4$. Under $H_0$, $hat(lambda)_"mle" = bar(X)_n approx cal(N)(4, 4\/n)$, so the standardized statistic is
$
Z = (bar(X)_n - 4)/sqrt(4\/100) approx cal(N)(0, 1) quad "under" H_0 .
$
...
(d) For $X_1, X_2, dots ~ "Poi"(2)$ the sample mean satisfies $bar(X)_n ->^("P") 2$.
]
#answer[
...
*(d) False.* The statement is missing #emph[independence]. Without it the weak law can fail: if $X_i = X_1$ for all $i$, then $bar(X)_n = X_1$ for every $n$, which does not converge to the constant $2$ (indeed $PP(|X_1 - 2| >= epsilon) > 0$ for suitable $epsilon$, since $PP(X_1 = k) > 0$ for all $k in NN$).
]
