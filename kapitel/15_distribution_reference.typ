...
Throughout, $p in [0, 1]$ is a success probability, $n, r in NN_(>0)$, and the Poisson parameter is $lambda in RR_(>=0)$; the geometric and negative-binomial rows require $p > 0$. For the hypergeometric $N in NN_(>0)$, $K in {0, dots, N}$ and $n in {0, dots, N}$. As in Chapter 3 we reuse the letter $p$ for both the parameter and the pmf, $k$ denotes its argument, and $NN = {0, 1, 2, dots}$. Boundary powers use $0^0:=1$. Each row gives a fixed state space; its positive-mass support can shrink at boundary parameters.

#table(
  columns: 5,
  align: (left, left, left, center, center),
  [*Distribution*], [*State space / support closure*], [*pmf* $p(k)$], [$EE[X]$], [$"Var"(X)$],
...
Parameters range over $a < b$, $mu in RR$, $sigma > 0$, $alpha, beta > 0$, $lambda > 0$, $k > 0$ and $nu > 0$. Each density is understood to be $0$ outside the stated state space or support closure. Two special functions appear in the densities below.
...
#table(
  columns: 5,
  align: (left, left, left, center, center),
  [*Distribution*], [*State space / support closure*], [*pdf* $p(x)$], [$EE[X]$], [$"Var"(X)$],
  ...
  [$"Beta"(alpha, beta)$], [$[0, 1]$], [$frac(x^(alpha - 1) (1 - x)^(beta - 1), B(alpha, beta))$ for $0<x<1$; endpoint values arbitrary], [$frac(alpha, alpha + beta)$], [$frac(alpha beta, (alpha + beta)^2 (alpha + beta + 1))$],
)

#remark[
The continuous families are also nested: $"Exp"(lambda) = Gamma(1, lambda)$; $chi_k^2 = Gamma(k\/2, 1\/2)$; $"Unif"(0, 1) = "Beta"(1, 1)$; and $t_nu$ is the law of $Z \/ sqrt(W \/ nu)$ for independent $Z ~ cal(N)(0, 1)$ and $W ~ chi_nu^2$. For integer $k>0$, $chi_k^2$ is the sum of $k$ independent squared standard normals. The Gamma family uses the #emph[rate] parameter $beta$ (so its mean is $alpha\/beta$, not $alpha beta$); for the mean of $t_nu$ to exist we need $nu > 1$, and for its variance $nu > 2$.
]
...
#keyfact[
For integrable $Y$, the law of total expectation is
$
EE[Y]=EE[EE[Y|X]],
$
and for square-integrable $Y$, the law of total variance is
$
...
The identities you reach for when computing with the distributions above. All expectations displayed below are assumed to exist. LOTUS requires $EE[abs(g(X))] < oo$; the law of total variance above requires $EE[Y^2] < oo$; and the displayed two-event Bayes formula requires $PP(B)>0$ and $PP(A)>0$. The two tail inequalities bound probabilities using only $EE$ and $"Var"$, and the two limit theorems describe the average $macron(X)_n = frac(1, n) sum_(i=1)^n X_i$ of i.i.d. samples.
...
#table(
  ...
  [LOTUS (discrete)], [$EE[g(X)] = sum_k g(k) p(k)$ if $EE[abs(g(X))]<oo$], [6],
  [LOTUS (continuous)], [$EE[g(X)] = integral_RR g(x) p(x) dif x$ if $EE[abs(g(X))]<oo$], [6],
  [Bayes' theorem], [$P(A | B) = frac(P(B | A) P(A), P(B))$ for $P(B)>0$, $P(A)>0$], [5],
...
#question[Give the pmf, state space, and positive-mass support of $"Poi"(lambda)$, its mean and variance, and one distribution it arises from as a limit.]
#answer[$p(k) = e^(-lambda) lambda^k \/ k!$ on the state space $NN = {0, 1, 2, dots}$ for $lambda >= 0$, with $0^0=1$ and $EE[X] = "Var"(X) = lambda$. Its positive-mass support is $NN$ for $lambda>0$ and ${0}$ for $lambda=0$; it is the $n -> infinity$ limit of $"Bin"(n, lambda\/n)$ — the law of rare events.]

#question[For $"Geo"(p)$, state the pmf, its state space and positive-mass support, and its mean and variance.]
#answer[$p(k) = (1 - p)^(k - 1) p$ on the state space $NN_(>0) = {1, 2, 3, dots}$, with $EE[X] = 1\/p$ and $"Var"(X) = (1 - p)\/p^2$. Its positive-mass support is all of $NN_(>0)$ when $0<p<1$ and ${1}$ when $p=1$. It is the special case $"NegBin"(1, p)$.]
...
#answer[$p(x) = frac(x^(alpha - 1) (1 - x)^(beta - 1), B(alpha, beta))$ for $0<x<1$ (and $0$ outside, with arbitrary endpoint values), with support closure $[0, 1]$, $EE[X] = frac(alpha, alpha + beta)$ and $"Var"(X) = frac(alpha beta, (alpha + beta)^2 (alpha + beta + 1))$. The uniform distribution $"Unif"(0, 1)$ is $"Beta"(1, 1)$.]
...
#answer[For every real $k>0$, $chi_k^2 = Gamma(k\/2, 1\/2)$ with density $frac(1, 2^(k\/2) Gamma(k\/2)) x^(k\/2 - 1) e^(-x\/2)$ on $(0, infinity)$, mean $k$ and variance $2k$. When $k$ is a positive integer, it is also the law of a sum of $k$ squared independent standard normals.]
